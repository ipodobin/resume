...
import {Book} from '../edit/edit.component';
...
export class ResumeService {
  ...
  static resolveUserObserver(value, results, observer) {
    ...
    return new Observable<User>(observer => {
      ...
      data$.subscribe(results => {
        ...
        this.db.object<User>(path).valueChanges().subscribe(value => {
          if (value != null) {
            ResumeService.resolveUserObserver(value, results, observer);
          } else if (default_path === false) {
            path = '/default/' + this.translate.currentLang;
            this.db.object<User>(path).valueChanges().subscribe(default_value => {
              if (default_value != null) {
                ResumeService.resolveUserObserver(default_value, results, observer);
              }
            });
          }
        });
  ...
  getResume(id: string): Observable<any> {
    // return new Observable<Resume>(observer => {
    const resume = this.store.collection(`/data/${this.auth.user.uid}/resumes`).doc(id).get();
    // });
    return resume;
  }
}
